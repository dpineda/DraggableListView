...
namespace DraggableListView.iOS
{
			...
			private static void OnRecognizing(LongPressGestureRecognizer recognizer, UITableView tableView, ViewCell cell)
			{
				...
				switch (recognizer.State)
				{
					...
					case UIGestureRecognizerState.Ended:
						...
						{
							if (DDListViewRenderer.ListMap.ContainsKey(tableView.Tag.ToString()) && DDListViewRenderer.ListMap.ContainsKey(destinationTableView.Tag.ToString()))
							{
								var sourceList = (IList)DDListViewRenderer.ListMap[tableView.Tag.ToString()].Item2;
								var destinationList = (IList)DDListViewRenderer.ListMap[destinationTableView.Tag.ToString()].Item2;
